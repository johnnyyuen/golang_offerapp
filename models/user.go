...
func (u *User) Register(conn *pgx.Conn) error {
	...
	if u.Password != u.PasswordConfirm {
		return fmt.Errorf("%v", "Passwords do not match.")
	}

	if len(u.Email) < 4 {
		return fmt.Errorf("%v", "Email must be at least 4 characters long.")
	}
	...
	if err != pgx.ErrNoRows {
		...
		return fmt.Errorf("%v", "A user with that email already exists")
	}

	pwdHash, err := bcrypt.GenerateFromPassword([]byte(u.Password), bcrypt.DefaultCost)
	if err != nil {
		return fmt.Errorf("%v", "There was an error creating your account.")
	}
...
func (u *User) IsAuthenticated(conn *pgx.Conn) error {
	...
	if err == pgx.ErrNoRows {
		...
		return fmt.Errorf("%v", "Invalid login credentails")
	}

	err = bcrypt.CompareHashAndPassword([]byte(u.PasswordHash), []byte(u.Password))
	if err != nil {
		return fmt.Errorf("%v", "Invalid login credentials")
	}
...
func IsTokenValid(tokenString string) (bool, string) {
	//fmt.Printf("inside IsTokenValid %v\n", tokenString)
